...
humidity_initial = 55
temperature_initial = 20

rasbpy_id = "1"
if len(sys.argv) > 1:
 rasbpy_id = str(sys.argv[1])
if len(sys.argv) > 2:
 temperature_initial = int(sys.argv[2])
if len(sys.argv) > 3:
 humidity_initial = int(sys.argv[3])
...
while(True):
  ...
  v = "{0:0.2f}".format(random.random()*10)
  humidity = humidity_initial + float(v)
  v = "{0:0.2f}".format(random.random()*5)
  temperature = temperature_initial + float(v)
  if humidity is not None and temperature is not None:
    # Find the device and its last ip
    res = col_devices.find("_id='{}'".format(rasbpy_id)).fields('_id', 'publicip').execute()
    ...
    patch_json['last_update'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print('Temp={}  Humidity={}'.format(temperature_s, humidity_s))
    col_devices.modify("_id='{}'".format(rasbpy_id)).patch(patch_json).execute()
    if old_public_ip != external_ip:
       tbl_publicip_history.insert(['device_id', 'ip_address']).values(rasbpy_id, external_ip).execute() 
    tbl_temperature_history.insert(['device_id', 'value']).values(rasbpy_id, temperature).execute() 
    tbl_humidity_history.insert(['device_id', 'value']).values(rasbpy_id, humidity).execute() 
  else:
